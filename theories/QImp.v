...
Inductive AResult : Type :=
  | ANormal : Number -> AResult
  | AError : AResult.

Inductive BResult : Type :=
  | BNormal : bool -> BResult
  | BError : BResult.
...
Fixpoint aeval (st : state)
               (a : aexp) : AResult :=
  match a with
  | AInt n => ANormal (IntVal n)
  | ARational f => ANormal (QVal f)
  | AId x => 
    match st x with
    | Some a => ANormal a
    | None => AError
  ...
  | <{a1 * a2}> => 
    ...
    | ANormal v1 =>
      match (aeval st a2) with
      | ANormal v2 => ANormal (NMult v1 v2)
      | A => A
      end
    | A => A
    end
